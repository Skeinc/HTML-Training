...
**Примечание:** Тег ``<colgroup>`` должен быть дочерним элементом ``<table>`` и должен располагаться перед любыми другими элементами таблицы, такими как ``<thead>``, ``<tr>``, ``<td>`` и т.д., но после тега ``<caption>``, если он присутствует.
